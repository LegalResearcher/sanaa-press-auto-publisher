...
_DATA_DIR = os.environ.get("BOT_DATA_DIR", os.path.join(_SCRIPT_DIR, "sanaa_press_data"))
LOCK_FILE_PATH = os.path.join(_DATA_DIR, "auto_publish_sanaa_press.lock")

sys.path.insert(0, _SCRIPT_DIR)

from sanaa_press_news_bot import (
    log,
    RSS_MASA_URL,
    RSS_MASA_CATEGORY,
...
SELECTED_FEEDS = {
    # فيد قسم "اليمن" فقط بوكالة الصحافة اليمنية — مُستبعد منه تلقائياً كل
    # أقسام الموقع الأخرى بحكم كونه فيد القسم نفسه، وتُنشر كل أخباره بقسم
    # "أخبار وتقارير" ثابتاً (RSS_YPAGENCY_YEMEN_CATEGORY):
...
def run():
    log.info("═" * 60)
    log.info("  📰  صنعاء برس — تشغيل تلقائي (ypagency اليمن + الجزيرة اليمن + المساء برس)")
    log.info("═" * 60)
    ...
    # مباشرة (قسم ypagency ثابت من الأصل "أخبار وتقارير"، وقسم الجزيرة يُصحَّح
    # تلقائياً بهذه المرحلة عبر apply_full_extraction لو تأكد أنه عن اليمن)
    ...
    for it in new_items:
        ...
        record = {
            ...
            # 🆕 نفس تعبئة sanaa_press_news_bot.py الرئيسي للأعمدة الجديدة بجدول posts.
