...
# ⚠️ أقسام صنعاء برس الفعلية بالجدول حالياً 5 فقط: "أخبار وتقارير"،
# "أخبار دولية"، "رياضة"، "اراءا واتجاهات" (لاحظ الإملاء المختلف عن
# "آراء واتجاهات" أعلاه — نفس القسم)، "منوعات وتكنولجيا". لا يوجد قسم مخصص
# لـ"أسعار العملات والذهب" (الموقع يعرضها عبر جدول market_rates كـ widget
# منفصل بالواجهة، تماماً كشمسان)، ولا لـ"أخبار عدن"/"أخبار محلية"/
# "أخبار اليمن"/"شؤون دولية"/"منوعات" بالتسمية الحرفية — كلها تُحوَّل أدناه
# لأقرب قسم فعلي. لو أضفت أقساماً جديدة لاحقاً بلوحة الإدارة، حدّث هذا
# القاموس فقط.
CATEGORY_NAME_REMAP = {
    ...
    "شؤون دولية": "أخبار دولية",
    "آراء واتجاهات": "اراءا واتجاهات",
    "منوعات": "منوعات وتكنولجيا",
}
...
BLOCKED_KEYWORDS_SOURCES = {RSS_YPAGENCY_YEMEN_URL}
...
def main():
    ...
    else:  # "1" — استخراج كامل: فيد ypagency (اليمن) الحي + فيد الجزيرة (اليمن) + فيد المساء برس معاً
        selected_feeds = {
