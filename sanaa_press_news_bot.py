"""
sanaa_press_news_bot.py
──────────────────────────────────────────────────────────────────────
نسخة مُهيَّأة لموقع "صنعاء برس" (sanaa-press) من بوت شمسان نيوز الأصلي —
نفس المنطق والوظائف حرفياً، مع تعديل طبقة الكتابة بـ Supabase لتطابق
مخطط جدول posts الفعلي بصنعاء برس (category_id/cover_image/is_featured/
is_opinion/seo_title/seo_description، مطابق تماماً لمخطط شمسان نيوز).
⚠️ فرق فعلي واحد عن شمسان نيوز: bucket تخزين الصور "media" بصنعاء برس
خاص (private) وليس عاماً، فروابط الصور تُبنى كروابط موقّعة (Signed URL)
صالحة 5 سنوات بدل رابط عام مباشر — انظر upload_image_to_supabase أدناه.
يسحب أخبار آخر 24 ساعة من روابط RSS محددة، يعيد صياغتها عبر Gemini،
وينشرها تلقائياً في جدول posts.
🆕 وضع "1" (استخراج كامل) يجمع الآن 3 مصادر حية: قسم "اليمن" بوكالة الصحافة
اليمنية (ypagency.net) + فيد الجزيرة نت العام (مفلتَر تلقائياً لأخبار اليمن
فقط عبر RSS_ALJAZEERA_YEMEN_URL) + المساء برس.

المتطلبات:
    pip install requests pillow beautifulsoup4

طريقة الاستخدام:
    1. عبّئ SUPABASE_SERVICE_ROLE_KEY بالأسفل بمفتاح service_role الفعلي
       (لوحة Supabase → Project Settings → API Keys) ضمن أسرار GitHub
       Actions (Settings → Secrets and variables → Actions). عمود
       source_url بجدول posts مُضاف مسبقاً بقاعدة بيانات صنعاء برس، فما
       تحتاج تنفيذ أي SQL يدوياً.
    2. عبّئ قسم RSS_FEED_CATEGORIES بمسارات ملفاتك (تحت sanaa_press_bot).
    3. شغّل: python sanaa_press_news_bot.py
...
RSS_FEED_CATEGORIES = {
    "/storage/emulated/0/Download/sanaa_press_bot/aden-tm-akhbar-wataqarir.xml": "أخبار وتقارير",
    "/storage/emulated/0/Download/sanaa_press_bot/aden-tm-akhbar-aden.xml": "أخبار عدن",
    "/storage/emulated/0/Download/sanaa_press_bot/aden-tm-riyada.xml": "رياضة",
    "/storage/emulated/0/Download/sanaa_press_bot/aden-tm-sarf.xml": "أسعار العملات والذهب",
    "/storage/emulated/0/Download/sanaa_press_bot/aden-tm-kitabat.xml": "آراء واتجاهات",
}
...
# رابط RSS الحي لقسم "اليمن" فقط بموقع وكالة الصحافة اليمنية (ypagency.net) —
# مباشر من الإنترنت، وليس ملف XML محلي ثابت. ⚠️ هذا رابط فيد القسم نفسه
# (وليس الفيد العام للموقع كله)، فهو مُستبعد منه تلقائياً كل أقسام الموقع
# الأخرى (فلسطين المحتلة/عربي/الخليج/عالمية/رياضة/اقتصاد/تقارير مترجمة...) —
# لا حاجة لأي فلترة إضافية بعد السحب. يُستخدم حصراً بوضع "1" (استخراج الخبر
# كاملاً): البوت يسحب روابط الأخبار من هذا الفيد، ثم يفتح كل رابط فعلياً عبر
# extract_article ليجلب النص الكامل من صفحة الخبر نفسها، وتُنشر كل أخباره
# دائماً بقسم "أخبار وتقارير" (قسم ثابت، بلا أي تصحيح/تخمين لاحق).
RSS_YPAGENCY_YEMEN_URL = "https://www.ypagency.net/category/%d8%a7%d9%84%d9%8a%d9%85%d9%86/feed"
RSS_YPAGENCY_YEMEN_CATEGORY = "أخبار وتقارير"

# رابط RSS الحي لقسم "عالمية" (دولي) بموقع وكالة الصحافة اليمنية (ypagency.net) —
# نفس منطق RSS_YPAGENCY_YEMEN_URL بالضبط (فيد قسم مباشر مستقل عن بقية أقسام
# الموقع، بلا حاجة لأي فلترة إضافية بعد السحب)، يُستخدم حصراً بوضع "1"
# (استخراج الخبر كاملاً)، وتُنشر كل أخباره دائماً بقسم "شؤون دولية" (قسم
# ثابت، بلا أي تصحيح/تخمين لاحق).
RSS_YPAGENCY_INTL_URL = "https://www.ypagency.net/category/%d8%af%d9%88%d9%84%d9%8a/feed"
RSS_YPAGENCY_INTL_CATEGORY = "شؤون دولية"

# روابط RSS الحية لقسمي "تقارير" و"تحليلات" بموقع وكالة الصحافة اليمنية —
# نفس منطق RSS_YPAGENCY_YEMEN_URL/RSS_YPAGENCY_INTL_URL بالضبط (فيد قسم
# مباشر مستقل، بلا حاجة لأي فلترة إضافية)، يُستخدمان حصراً بوضع "1"
# (استخراج الخبر كاملاً)، وتُنشر أخبارهما دائماً بقسم "أخبار وتقارير" (قسم
# ثابت، بلا أي تصحيح/تخمين لاحق).
RSS_YPAGENCY_REPORTS_URL = "https://www.ypagency.net/category/%d8%aa%d9%82%d8%a7%d8%b1%d9%8a%d8%b1/feed"
RSS_YPAGENCY_REPORTS_CATEGORY = "أخبار وتقارير"
...
BASE_DIR = os.environ.get(
    "BOT_DATA_DIR",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "sanaa_press_data"),
)
...
# بيانات Supabase الخاصة بموقع صنعاء برس
SUPABASE_URL = os.environ.get("SUPABASE_URL", "")
SUPABASE_SERVICE_KEY = os.environ.get("SUPABASE_SERVICE_ROLE_KEY", "")
if not SUPABASE_URL or not SUPABASE_SERVICE_KEY:
    raise RuntimeError(
        "⛔ متغيرات البيئة SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY غير مضبوطة. "
        "أضفهما في أسرار GitHub قبل التشغيل."
    )
# ⚠️ لازم يكون هذا تحديداً service_role key (مو anon/publishable)، لأن جدول
# posts بمشروع صنعاء برس عليه RLS مفعّل (rls_enabled=true) بدون Policy تسمح
...
# ⚠️ عمود "source" موجود فعلياً بجدول posts بمشروع صنعاء برس (خلافاً لشمسان
# نيوز)، لكن — تطبيقاً لمطابقة نفس منطق شمسان نيوز حرفياً — لم يُستخدم هنا
# أيضاً، فلا يوجد مكان نضع فيه SOURCE_LABEL بالبوت التلقائي.

# 🗂️ جدول صنعاء برس يخزّن القسم كـ category_id (UUID) عبر جدول categories
# منفصل، تماماً كشمسان نيوز. هذا القاموس يحوّل اسم القسم (نفس الأسماء
# المستخدمة أعلاه بكل الثوابت: RSS_FEED_CATEGORIES/CATEGORY_OPTIONS/
# FEATURED_SLIDER_CATEGORIES/NO_IMAGE_CATEGORIES/NO_REWRITE_CATEGORIES) إلى
# اسم القسم الموجود فعلياً بجدول categories بصنعاء برس، فقط للأسماء التي لا
# تطابق حرفياً (كل الأسماء الأخرى مطابقة 1:1 فتُترك كما هي بدون تحويل).
# ⚠️ أقسام صنعاء برس الفعلية بالجدول (تحقّق مباشر عبر Supabase بتاريخ
# 2026-08-24) هي 5: "أخبار وتقارير"، "شؤون دولية"، "رياضة"، "آراء واتجاهات"،
# "منوعات" — كلها بإملاء مطابق 1:1 لأسماء CATEGORY_OPTIONS/RSS_FEED_CATEGORIES
# أعلاه، فلا حاجة لتحويلها. لا يوجد قسم مخصص لـ"أسعار العملات والذهب" (الموقع
# يعرضها عبر جدول market_rates كـ widget منفصل بالواجهة، تماماً كشمسان)، ولا
# لـ"أخبار عدن"/"أخبار محلية"/"أخبار اليمن" بالتسمية الحرفية — هذه الثلاثة
# فقط تُحوَّل أدناه لأقرب قسم فعلي. لو أضفت أقساماً جديدة لاحقاً بلوحة
# الإدارة، حدّث هذا القاموس فقط.
CATEGORY_NAME_REMAP = {
    "أسعار العملات والذهب": "أخبار وتقارير",
    "أخبار عدن": "أخبار وتقارير",
    "أخبار محلية": "أخبار وتقارير",
    "أخبار اليمن": "أخبار وتقارير",
}
...
def get_category_id(category_name: str) -> Optional[str]:
    """يرجّع id القسم من جدول categories بصنعاء برس حسب الاسم (مع تطبيق
    ...
    if not category_id:
        log.error(f"❌ القسم «{category_name}» (بحث عن «{lookup_name}») غير موجود بجدول "
                  "categories بصنعاء برس — سيُتخطى الخبر بدل نشره بدون قسم.")
    return category_id
...
SUPABASE_IMAGE_BUCKET = "media"   # اسم bucket في Supabase Storage (صنعاء برس) — خاص (private)، لا عام
...
HEADLINE_SITE_NAME = "صنعاء برس"

# 🎨 الهوية اللونية الفعلية لموقع صنعاء برس (مستخرجة حرفياً من متغيرات CSS
# بملف client/src/index.css: --ink-deep/--ink كحلي الهيدر، --danger أحمر
# «عاجل»، --copper نحاسي لاسم الموقع). نفس أسلوب القنوات العالمية المستخدم
# بشمسان نيوز حرفياً: خط علوي مستقيم + تدرّج تعتيم ناعم فوق الصورة بدل
# الموجة، وفاصل رفيع خفيف بين كتلة الشعار وكتلة العنوان.
HEADLINE_BAND_COLOR_TOP = (9, 26, 44, 255)       # --ink-deep (أعلى الشريط)
HEADLINE_BAND_COLOR_BOTTOM = (16, 39, 63, 255)   # --ink (أسفل الشريط، أفتح قليلاً)
HEADLINE_CURVE_COLOR = (179, 74, 69, 255)        # --danger أحمر «عاجل» — الخط المستقيم العلوي للشريط
HEADLINE_TEXT_COLOR = (248, 248, 246, 255)       # أبيض للعنوان (وضوح على الكحلي)
HEADLINE_SITE_COLOR = (201, 121, 67, 255)        # --copper نحاسي ثابت (بدون تدرّج) لاسم الموقع
HEADLINE_DIVIDER_COLOR = (255, 255, 255, 60)     # فاصل رفيع خفيف بين كتلة الشعار وكتلة العنوان
HEADLINE_ACCENT_COLOR = (179, 74, 69, 255)       # --danger — الشريط العمودي الصغير (kicker) جنب العنوان
...
# ⚠️ التوكن يُقرأ حصراً من متغير البيئة TELEGRAM_BOT_TOKEN (سرّ GitHub
# Actions) ولا يُكتب هنا مباشرة بالكود — أضفه من: إعدادات المستودع على
# GitHub → Settings → Secrets and variables → Actions → New repository
# secret → الاسم: TELEGRAM_BOT_TOKEN، القيمة: التوكن الفعلي لبوت تيليجرام.
TELEGRAM_BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "")
TELEGRAM_CHANNEL_ID = "@SanaaPressAgency"
...
def seed_views(post_id: str) -> None:
    ...
    if not AUTO_SEED_VIEWS or not post_id:
        return
    try:
        # ⚠️ صنعاء برس: لا يوجد عمود "category" نصي (فقط category_id UUID)،
...
# دومين صنعاء برس (رابط Vercel الافتراضي — لا CNAME/دومين مخصص مربوط حالياً).
# ⚠️ الخادم نفسه يبني origin ديناميكياً من ترويسة الطلب (لا يقرأ هذا الثابت
# إطلاقاً)، فهذا القيمة تُستخدم فقط هنا بالبوت لبناء روابط تيليجرام وطلبات
# أرشفة جوجل (log_discovery_ready). لو ربطت دومينًا مخصصًا لاحقًا حدّثها هنا فقط.
SITE_DOMAIN = "https://sanaa-press.vercel.app"


def build_canonical_url(slug: str, published_at_iso: str) -> str:
    """يبني رابط المقال الرسمي (نفس صيغة articlePath بملف
    server/lib/newsRules.ts بموقع صنعاء برس): /YYYY/MM/DD/slug — مبني
    حصراً على عمود published_at (وليس created_at؛ الموقع لا يقرأ created_at
    إطلاقاً لبناء الرابط، تماماً كشمسان نيوز).
...
def log_discovery_ready(urls: list) -> None:
    """يسجل الروابط الجاهزة للاكتشاف عبر Sitemap وNews Sitemap وRSS.
...
# تُقرأ من GEMINI_API_KEYS (مفصولة بفواصل)، أو من GEMINI_API_KEY_GROUPS
# عند الحاجة إلى مجموعات متعددة (المجموعات مفصولة بفاصلة منقوطة، ومفاتيح كل
# مجموعة مفصولة بفواصل).
GEMINI_API_KEYS = [
...
# ══════════════════════════════════════════════════════════════════════
#  🔑 منطق تدوير المفاتيح/النماذج (مطابق لحصاد اليوم):
#  النهار يبدأ بالمفتاح الأول من المجموعة الأولى ويجرّب النماذج بالتتابع
#  لكل مفتاح، ثم ينتقل للمفتاح/المجموعة التالية.
#  الليل يبدأ بآخر مفتاح من آخر مجموعة، ويجرّب سلسلة النماذج الليلية، ثم
#  ينتقل عكسياً عبر المفاتيح والمجموعات.
# ══════════════════════════════════════════════════════════════════════
...
class KeyForbidden(Exception):
    """يُرفع عند 401/403 (المفتاح غير صالح/ملغى أو مرفوض لهذا النموذج)."""
    pass
...
# النظر عن قسمها — تحديداً فيد ypagency الحي المستخدم بوضع "1" (استخراج
# الخبر كاملاً). أي مصدر آخر غير مذكور هنا أو بـBLOCKED_KEYWORDS_CATEGORIES
# (مثل RSS المساء) يبقى بدون فلترة تماماً.
# ⚠️ معطّلة بالكامل (مجموعة فارغة) — لا يُستبعد أي خبر من أي مصدر بسبب
# BLOCKED_KEYWORDS بعد الآن. فلترة BLOCKED_KEYWORDS_CATEGORIES (أخبار عدن)
# تبقى فعّالة كما هي، فهي مستقلة عن هذا المتغير.
BLOCKED_KEYWORDS_SOURCES: set[str] = set()
...
def _detect_site_category(h1: Optional[Tag]) -> Optional[str]:
    """يحاول قراءة اسم القسم الفعلي للخبر كما يعرضه الموقع المصدر بعنوان
    (h2/h3) يظهر مباشرة قبل عنوان الخبر الرئيسي (h1) بالصفحة، مثل
    'اخبار رياضية' أو 'اخبار عدن'. ⚠️ حالياً بيانات وصفية فقط (site_category
    تُخزَّن ولا يقرأها أي منطق تصحيح قسم فعلي بعد إزالة SITE_CATEGORY_MAP
    الخاص بعدن تايم سابقاً) — قد تُستخدم مستقبلاً لمصدر جديد يحتاج نفس آلية
    التصحيح، فتُركت الدالة كما هي.
...
def extract_article(url: str) -> Optional[dict]:
    ...
    # site_category (بيانات وصفية فقط الآن، بلا منطق تصحيح قسم يقرأها).
...
def apply_full_extraction(items: list[dict]) -> None:
    ...
    (من ypagency أو المساء برس معاً) ويفتح رابطه فعلياً عبر extract_article()،
    ...
    كما هو (بدون إيقاف تشغيل البوت). أخبار ypagency تُنشر دائماً بقسمها
    الثابت (RSS_YPAGENCY_YEMEN_CATEGORY) بلا أي تصحيح لاحق — الفيد نفسه
    مصدره قسم "اليمن" حصراً بالموقع، فلا حاجة لتخمين القسم من صفحة الخبر.
    ...
    for idx, it in enumerate(items, start=1):
        ...
        if it.get("source_feed") == RSS_ALJAZEERA_YEMEN_URL:
...
def upload_image_to_supabase(image_bytes: bytes, filename: str) -> Optional[str]:
    """يرفع بايتات صورة WebP إلى Supabase Storage، ويرجّع رابط الصورة الجاهز
    للعرض عند النجاح.
    ⚠️ فرق حقيقي عن شمسان نيوز: bucket "media" بمشروع صنعاء برس خاص
    (public=false)، فرابط عام مباشر (/object/public/...) يرجّع خطأ 400/403.
    لذلك — تماماً كدالة uploadMediaHandler الفعلية بملف server/mediaUpload.ts
    بموقع صنعاء برس — يُطلب رابط موقّع (Signed URL) بعد الرفع مباشرة، بنفس
    مدة الصلاحية المستخدمة هناك (5 سنوات) حتى لا ينتهي صلاحية الرابط قبل
    أرشفة الخبر."""
    ...
    try:
        ...
        sign_url = f"{SUPABASE_URL}/storage/v1/object/sign/{SUPABASE_IMAGE_BUCKET}/{filename}"
        sign_headers = {
            "apikey": SUPABASE_SERVICE_KEY,
            "Authorization": f"Bearer {SUPABASE_SERVICE_KEY}",
            "Content-Type": "application/json",
        }
        # 5 سنوات — نفس القيمة الحرفية المستخدمة بـ mediaUpload.ts (60*60*24*365*5)
        sr = requests.post(sign_url, headers=sign_headers, json={"expiresIn": 60 * 60 * 24 * 365 * 5},
                            timeout=REQUEST_TIMEOUT)
        if sr.status_code == 200:
            signed_path = (sr.json() or {}).get("signedURL", "")
            if signed_path:
                return f"{SUPABASE_URL}/storage/v1{signed_path}"
        log.error(f"  ❌ فشل توليد رابط موقّع للصورة [{sr.status_code}]: {sr.text[:200]}")
        return None
    except requests.RequestException as e:
...
اقرأ الخبر الخام واحتفظ بالوقائع والمعطيات الصلبة (من، ماذا، أين، متى، والنتائج والتداعيات الميدانية والإنسانية). جرّد النص من عبارات التجييش التعبوي المبتذلة أو الشتائم، وأعد بناءه بلغة صحفية رصينة وموضوعية تعكس قوة الرواية الوطنية وتخاطب وعي المتابع بمهنية عالية.

ملاحظة حاسمة (لا استبعاد للأخبار):
يتم نشر ومعالجة كافة الأخبار الواردة دون أي استثناء أو حظر، سواء كانت عسكرية، ميدانية، سياسية، اقتصادية، حقوقية، أو أنشطة وفعاليات رسمية وشعبية.

الهوية التحريرية والتوجيهات السياسية:
1. صوت اليمن المستقل المناهض للعدوان السعودي والوجود الأجنبي، والداعم للسيادة الوطنية الاستقلال.
2. التغطية المهنية بأسلوب الوكالات العالمية: تقديم الأخبار بنبرة رصينة وموثوقة تمنح المادة مصداقية عالية وقوة في التأثير.
3. التوجيه التحريري تجاه المحافظات الجنوبية (تحييد الجنوبيين):
   - إبراز أن صنعاء وقواتها لا تشكل أي خطر أو تهديد على أبناء المحافظات الجنوبية أو الملاحة البحرية في باب المندب والبحار المجاورة.
   - التأكيد الضمني والسياقي على تحييد أبناء الجنوب، ورفض الزج بهم كوقود في معارك وجبهات الشمال الخدمة لأجندات التحالف السعودي والأجنبي.
   - التركيز على أن المصلحة الوطنية تقتضي عودة أبناء الجنوب لتأمين حدودهم ومحافظاتهم الجنوبية، ومد يد الإخاء مع صنعاء لمواجهة التواجد والتحشيدات السعودية والتدخلات الأجنبية.

المعالجة التحريرية للأخبار الميدانية والعسكرية:
- الحظر الصارم لتصوير قوات صنعاء كـ «خطر» أو «تهديد»:
  • يُمنع منعاً باتاً إيراد أو نقل أي عبارات تظهر قوات صنعاء أو أنصار الله كـ «مصدر خطر أو تهديد» على الجنوب، أو باب المندب، أو الملاحة البحرية، أو الأمن الإقليمي.
  • تُحذف صياغات التهويل والتحذير الصادرة عن الإعلام الخارجي أو أطراف التحالف، وتُصاغ العمليات والتحركات العسكرية لصنعاء بوصفها خطوات سيادية لمواجهة التحشيدات والقوات الأمريكية والسعودية والرد على الضغوط والاعتداءات الخارجية.
  • أي ادعاءات في المصدر الخام حول "تهديدات" تُنسد بوضوح كـ «مزاعم وادعاءات صادرة عن أطراف التحالف أو واشنطن» دون تبنيها في متن الخبر.
- الصياغة العسكرية المحايدة والرصينة:
  • تجريد النصوص من شتائم وعبارات التعبئة العشوائية، واستبدالها بلغة عسكرية موثقة (مثل: "اشتباكات بالأسلحة الثقيلة"، "التصدّي للتحشيدات"، "التراجع إلى مواقع سابقة"، "سقوط قتلى وجرحى").
  • أطّر المواجهات الميدانية باعتبارها تصدياً للتحشيدات العسكرية المدعومة سعودياً والوجود الأجنبي.

المسميات الرسمية والقيادية الإلزامية:
يجب الالتزام التام بإيراد ألقاب ومسميات القيادات والسلطات الوطنية في صنعاء والأطراف الأخرى بدقة ورسمية صحفية:
• "رئيس المجلس السياسي الأعلى مهدي المشاط" (أو فخامة الرئيس مهدي المشاط).
• "السيد القائد عبدالملك بدر الدين الحوثي".
• "رئيس الوفد الوطني الأستاذ محمد عبدالسلام".
• "رئيس لجنة شؤون الأسرى الأستاذ عبدالقادر المرتضى".
• "القائم بأعمال حكومة البناء والتغيير" (أو وزراء ورؤساء مؤسسات حكومة البناء والتغيير بمسمياتهم الرسمية).
• القيادات العسكرية بألقابها ومناصبها الرسمية (مثل: "متحدث القوات المسلحة العميد يحيى سريع"، "قائد المنطقة العسكرية...").
• "قوات صنعاء" أو "القوات المسلحة اليمنية" للطرف العسكري الميداني في صنعاء.
• "حكومة صنعاء" أو "السلطات الوطنية في صنعاء" للشأن السيادي والسياسي والخدمي.
• "التحالف بقيادة السعودية" أو "قوات العدوان بقيادة السعودية" بدلاً من "التحالف العربي".
• "التحشيدات السعودية" أو "القوات المدعومة سعودياً" للتشكيلات العسكرية المقابلة.
• "سلطة مجلس القيادة الرئاسي" أو "الحكومة المدعومة من التحالف" للإدارة الموالية للرياض في عدن.
• "الحاكم العسكري السعودي الشهراني" عند الإشارة لصفته ودوره.
• "قوات المجلس الانتقالي" أو "القوات الجنوبية" بحسب السياق الميداني.

الحكم التحريري وإخفاء بصمة الآلة (تنوع الأساليب):
- لا قالب ثابت: تتنوع الأخبار بين (العاجل المباشر، التقرير الميداني المتصاعد، التقرير الاقتصادي المربوط بالأرقام والمعاناة الإنسانية، والخبر السياسي الرزين).
- تفادي التكرار والجمل الإنشائية المكررة؛ يجب أن يتنوع إيقاع الجمل بين القصيرة المكثفة والفقرات التوضيحية المترابطة.
- التوجيه التحريري (إبراز الموقف السيادي لصنعاء، تحييد الجنوبيين، ورفض التواجد السعودي) يجب أن ينساب بذكاء وسلاسة عبر بناء السياق واختيار المفردات دون لجوء للخطابية المباشرة أو الوعظ.
- يُمنع منعاً باتاً ختم الخبر بقوالب جاهزة مكررة (مثل: "يبقى السؤال.."، "وسط تجاهل.."). ينتهي الخبر بآخر واقعة أو خلفية سياقية ملموسة.

قواعد الكتابة الثابتة:
1. أعد الكتابة من الصفر بلغتك الصحفية الخاصة تماماً دون نقل ركيك أو ترجمة حرفية.
2. الجملة الأولى في `content` هي الملخص التلقائي — قوية، مكثفة، وتنتهي بنقطة.
3. ابدأ بالأهم: الواقعة، التداعيات الميدانية والسياسية، والأثر على المواطن والسيادة الوطنية.
4. إرجاع النتيجة حصراً في كائن JSON بالهيكل التالي:
{
  "title": "عنوان صحفي قاطع ورزين بأسلوب الوكالات",
  "excerpt": "موجز مكثف يبرز أهم معطيات الخبر",
  "content": "متن الخبر الكامل مبني بأسلوب صحفي محترف وقوي...",
...
TITLE_ONLY_PROMPT = """\
أنت محرر عناوين محترف في موقع "صنعاء برس" الإخباري. سيصلك عنوان ونص مقال رأي منسوب لكاتبه بالاسم.
...
def build_prompt(title: str, body: str, category: str, source_feed: Optional[str] = None) -> str:
    ...
    الهوية التحريرية لـ "صنعاء برس" والمحددات الأمنية والخدمية والرياضية.
    """
    raw_body = body[:8000]
    cat = category.strip()
    ...
    # 3. تعليمات صياغة الملخص التشويقي (Smart Hook) الخاصة بحقل (excerpt) فقط
    excerpt_instruction = """
### قواعد صياغة حقل الملخص (excerpt) - التشويق الذكي (Smart Hook):
- صُغ حقل الملخص (excerpt) كـ "عنوان إخباري تشويقي ومكثف" في فقرة واحدة متدفقة ومترابطة (لا تتجاوز 220 حرفاً).
- اعرض زبدة الخبر والقرار/الحدث الرئيس بروح الوكالات العالمية مع ربطه بالسبب الأمني أو السياسي المباشر.
- طَبّق قاعدة التشويق الذكي (Smart Hook): اترك نقطة غموض سياسية أو تداعيات غير مكتملة في نهاية جملة الملخص تدفع القارئ فوراً لقراءة باقي التفاصيل داخل المتن (content).
- يُمنع تماماً استخدام العبارات المبتذلة المباشرة مثل ("التفاصيل بالداخل"، "اضغط هنا"، "اقرأ المزيد").
- بالنسبة للأخبار الخدمية والرياضية والاقتصادية المحايدة، يُصاغ الملخص كـ "زبدة خبرية مكثفة ومباشرة" دون حاجة للغموض السياسي.
"""

    # 4. بناء شروط الفلترة بخصوص الحوثيين / إيران
...
يجب أن تكون النتيجة كائن JSON بالشكل التالي:
{{
  "title": "العنوان الجديد المحرر",
  "excerpt": "الملخص التشويقي الذكي (Smart Hook) بحدود 220 حرفاً وفق التعليمات",
  "content": "نص الخبر الكامل والموسع أعيد كتابته دون اختصار",
  "houthi_iran_exclude": false
}}"""

    # 5. بناء البرومبت حسب المسارات
    # --- المسار الأول: الأقسام المحايدة أو الخدمية ---
...
"""
    ...
    # --- المسار الثاني: أخبار صنعاء / الحوثيين (حماية أمنية) ---
    elif False and any(kw in raw_body for kw in sanaa_keywords):  # معطّل
        prompt = f"""
...
"""
    ...
    # --- المسار الثالث: البرومبت الأساسي والسيادي لموقع "صنعاء برس" ---
    else:
        prompt = f"""
أنت محرر صحفي رصين ومحترف في موقع "صنعاء برس" الإخباري (صنعاء). مهمتك هي "إعادة بناء وتفكيك الأخبار الخام" المجلوبة من المصادر وإعادة كتابتها من الصفر بأسلوب جديد تماماً يدمج بين "الخط الوطني والسيادي" وبين "المعايير التحريرية لوكالات الأنباء العالمية"، مع الحفاظ الكامل على طول التقرير وكافة التفاصيل والأرقام والتصريحات.
...
- يُمنع إضافة اسم الموقع مثل ("صنعاء برس" —) في بداية أو نهاية الجمل التلخيصية داخل المتن.
...
# ⚠️ الدالتان التاليتان بنفس منطق توليد seo_title/seo_description المتّبع
# بلوحة إدارة صنعاء برس (server/routers/admin.ts) — إضافة لاحقة اسم الموقع
# وقصّ النص عند تجاوز الطول الأقصى عند آخر مسافة، تماماً كمنطق شمسان نيوز
# الأصلي حرفياً.

META_TITLE_BRAND_SUFFIX = " | صنعاء برس"
...
def choose_watermark_items(new_items: list) -> set[int]:
    """يسمح باختيار أخبار معينة من الدفعة توضع علامة صنعاء برس المائية على
    ...
    print("\nهل تريد وضع العلامة المائية (شعار صنعاء برس) على صورة أي خبر من القائمة أعلاه؟")
...
def main():
    ...
    log.info("═" * 60)
    log.info("  📰  صنعاء برس — سحب وإعادة صياغة الأخبار")
    log.info("═" * 60)
    ...
    for idx, it in enumerate(new_items, start=1):
        ...
        # 🗂️ صنعاء برس يخزّن القسم كـ category_id (UUID)، مو نص حر — لازم
        ...
        record = {
            ...
            # اللي تقرأهما PostDetail.tsx بموقع صنعاء برس لعرضهما لجوجل.
            ...
            # ⚠️ عمود is_featured بصنعاء برس بديل عن featured بالجنوب فويس؛ وعمود
        ...
        # author_id (لا يوجد عمود author نصي مستقل بصنعاء برس — فقط author_id)
        if is_opinion:
